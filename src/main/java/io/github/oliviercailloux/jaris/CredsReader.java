 ...
 * </p>
 * <p>
 * Allows to read the credentials: username and password. For each piece of information,
 * distinguishes <em>missing information</em> and <em>empty string</em>. Considers the following
 * possible sources (displayed here by order of priority).
 * </p>
 * <ol>
 * <li>Properties {@value #usernameKey} and {@value #passwordKey}. Each property may be set,
 * including to the empty string, or not set. An information is considered missing (from the
 * properties source) iff the corresponding property is not set.</li>
 * <li>Environment variables {@value #usernameKey} and {@value #passwordKey}. Each variable may be
 * set, including to the empty string, or not set. An information is considered missing (from the
 * environment variables source) iff the corresponding environment variable is not set.</li>
 * <li>File {@value #filePath}. The two pieces of information are considered missing (from the files
 * source) iff the file does not exist. If the file exists, no piece of information is considered
 * missing. The first line of the file gives the username, the second one gives the password. If the
 * file has only one line, the password (from the files source) is set to the empty string. If the
 * file is empty, both pieces of information (from the files source) are set to the empty string.
 * Empty lines are not considered at all. If the file has non empty line content after the second
 * line, it is an error.</li>
 * </ol>
 * <p>
 * Best login information: The source used to return information is the one that has the highest
 * informational value, as determined by {@link CredsOpt#getInformationalValue()} (meaning that
 * sources are ordered by increasing number of pieces of information missing), and, in case of
 * ex-æquo, the order of priority displayed in the previous paragraph determines which source wins.
 * </p>
 * </p>
 */
public class CredsReader {
  ...
  /**
   * The default value of the username key used in <code>CredsReader.defaultCreds()</code>.
   */
  public static final String DEFAULT_USERNAME_KEY = "API_username";

  /**
   * The default value of the password key used in <code>CredsReader.defaultCreds()</code>.
   */
  public static final String DEFAULT_PASSWORD_KEY = "API_password";

  /**
   * The default value of the file path used in <code>CredsReader.defaultCreds()</code>.
   */
  ...
  /**
   * @return a CredsReader instance that will read from the default values DEFAULT_USERNAME_KEY,
   *         DEFAULT_PASSWORD_KEY, DEFAULT_FILE_NAME.
   */
  public static CredsReader defaultCreds() {
    CredsReader credsReader =
        new CredsReader(DEFAULT_USERNAME_KEY, DEFAULT_PASSWORD_KEY, DEFAULT_FILE_NAME);
    return credsReader;
  }
  ...
  /**
   * <p>
   * Returns the best authentication information it could find, throwing no error if some is
   * missing.
   * </p>
   *
  ...
  CredsOpt readCredentials() throws IOException, IllegalStateException {
    final CredsOpt propertyAuthentication;
    {
      ...
      LOGGER.info("Found {} piece" + (informationalValue >= 2 ? "s" : "")
          + " of login information in properties.", informationalValue);
    }
