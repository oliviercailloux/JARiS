...
== Xml
https://www.javadoc.io/doc/io.github.oliviercailloux/jaris/latest/io/github/oliviercailloux/jaris/xml/XmlUtils.html[`XmlUtils`] is a utility class that eases managing a W3C DOM document and transforming and checking the validity of XML documents. For example, it provides a method to deal with a list of nodes as a Java `List` rather than the more cumbersome `NodeList` provided by the standard.
