...
The API is _not_ stable. Write to me if you are interested in using this library and would like its API to be stable, or want more example code.
...
[source, java]
----
StreamSource docBookSchema = new StreamSource("https://cdn.docbook.org/schema/5.0.1/xsd/docbook.xsd");
...
ConformityChecker conformityChecker = SchemaHelper.schemaHelper().getConformityChecker(docBookSchema);
...
String transformed = XmlTransformer.usingFoundFactory().forSource(myStyle).transform(docBook);
----
