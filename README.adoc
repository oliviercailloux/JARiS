...
== TODO
Resolve inconsistency: SchemaHelper#asSchema(ByteSource schemaSource) throws XmlException (modifies IOException) while helper.conformityChecker(compiledSchema).verifyValid(documentSource) throws IOException.
